...
public class RecipeDao {
    ...
    private static final String FIND_ALL_RECIPIES_QUERY = "SELECT * FROM recipe;";
    ...
    public List<Recipe> findAllByAdminId(Integer adminId) {
        ...
        try (Connection connection = DbUtil.getConnection();
             PreparedStatement statement = connection.prepareStatement(READ_RECIPE_BY_ADMINID_QUERY);
             ) {
            statement.setInt(1, adminId);
            try (ResultSet resultSet = statement.executeQuery()) {
                while (resultSet.next()) {
    ...
    public List<Recipe> findAll() {
        List<Recipe> recipeList = new ArrayList<>();
        try (Connection connection = DbUtil.getConnection();
             PreparedStatement statement = connection.prepareStatement(FIND_ALL_RECIPIES_QUERY);
